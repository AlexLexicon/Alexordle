...
//    //    int totalRows = Math.Max(puzzle.MaxGuesses, committedGuessesCount) + (!state.IsVictory && !state.IsDefeat && state.IsBonus ? 1 : 0);
...
//    //    HintTypes highlight = await highlightService.ProcessHighlightAsync(invariantCharacter, position, isCommitted, isDesignAnswers);
